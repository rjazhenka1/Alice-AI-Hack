...
**Главная ценность Алисы:** она не просто чат — это помощник-координатор штаба соревнований.
Её основная задача — соединять людей друг с другом и соединять людей с информацией.
Понимает неструктурированный текст ("на входе толпа"), маршрутизирует задачи по ролям,
...
1. **Однозначный операционный запрос → точный tool call → создание/назначение задачи**
   ...
   - Дополнительное доменное правило маршрутизации:
     - если запрос вида `"Нужны люди на входе из регистрации"`,
       задача создаётся на роль/команду **Регистрация** (а не абстрактно "на всех").
...
4. **Запрос к знаниям о мероприятии → ссылка только при наличии данных в админке**
   - Пример запроса: `"Где регламент по дисквалификациям?"`
   - Ожидаемое поведение:
     - если данные есть в админке, агент возвращает ссылку/указатель на соответствующий материал,
      - если данных нет в админке, агент не выдумывает, а уточняет или отвечает безопасным сообщением без фиктивных ссылок.

5. **Расплывчатый запрос (`imprecise`) → второй проход в уточнение**
   - Пример запроса: `"Сделай это как-нибудь"`
   - Ожидаемое поведение:
     - primary-классификация может вернуть `kind=imprecise`,
     - `router.py` выполняет второй проход с усиленной инструкцией,
     - финальный ответ пользователю — `question_asked` с одним конкретным уточняющим вопросом,
     - side effects (создание тикета/назначение) не выполняются.

6. **Закрытие задачи фразой координатора**
   - Пример: `"я сделал такую-то задачу"` или `"я сделал"`
   - Ожидаемое поведение:
     - агент находит релевантный активный тикет в видимой области,
     - помечает его как `resolved`,
     - если идентифицировать задачу нельзя — задаёт уточняющий вопрос.
...
- Best-effort KB-контекст из последних публичных сообщений админов
- Сводка видимых (non-confidential) тикетов
- Последние сообщения диалоговой памяти (`agent_sessions`, до 20)
